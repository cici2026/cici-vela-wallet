// Ported from src/services/selector-registry.ts @ f9bcb278 — RN seams rewritten to the web modules; logic verbatim.
...
const OPENCHAIN_HOSTS = ['https://api.4byte.sourcify.dev', 'https://api.openchain.xyz'];
...
async function fromOpenChainHost(host: string, selector: string): Promise<string[]> {
	const data = await getJson(
		`${host}/signature-database/v1/lookup?function=${selector}&filter=true`
	);
...
export async function lookupSelector(selectorHex: string): Promise<string[]> {
	const selector = (selectorHex.startsWith('0x') ? selectorHex : '0x' + selectorHex)
		.slice(0, 10)
		.toLowerCase();
