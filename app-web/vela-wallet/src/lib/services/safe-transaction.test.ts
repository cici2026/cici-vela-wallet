...
describe('safe-transaction', () => {
	describe('sameAssetFeeLimit', () => {
		...
		test('reserves the native fee only for a native transfer', () => {
			const nativeFee: TransactionFeeEstimate = {
				...erc20Fee,
				totalWei: 42n,
				feeAsset: { kind: 'native' }
			};
			expect(sameAssetFeeLimit(nativeFee, null, 100n)).toEqual({
				feeAmount: 42n,
				maxTransferAmount: 58n
			});
		...
		test('clamps the maximum to zero when the fee itself exhausts the balance', () => {
			expect(sameAssetFeeLimit(erc20Fee, USDT, 69_800n)).toEqual({
				feeAmount: 69_800n,
	...
	describe('buildExecuteCallData (Safe executeUserOp)', () => {
		...
		test('empty data → selector + 5 words, CALL operation, 0x80 data offset', () => {
			...
			expect(hex(out).slice(0, 8)).toBe(
				hex(functionSelector('executeUserOp(address,uint256,bytes,uint8)'))
			);
	...
	describe('buildInitCode (createProxyWithNonce)', () => {
		...
		test('is factory-address ++ createProxyWithNonce(...) and is deterministic', () => {
			...
			expect(selectorAfterFactory).toBe(
				hex(functionSelector('createProxyWithNonce(address,bytes,uint256)'))
			);
...
import { computeWebauthnSignerAddress, toHex, WEBAUTHN_SIGNER } from '$lib/core/kernels';
