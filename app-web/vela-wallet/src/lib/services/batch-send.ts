// Ported from src/services/batch-send.ts @ f9bcb278 — RN seams rewritten to the web modules; logic verbatim.
/**
...
export function encodeErc20Transfer(to: string, amount: bigint): string {
	...
	return (
		'0x' + ERC20_TRANSFER_SELECTOR + pad32(to.slice(2).toLowerCase()) + pad32(amount.toString(16))
	);
}
...
export function buildMultiTokenCalls(recipient: string, tokens: MultiTokenSpec[]): BatchCall[] {
	...
	return tokens.map((tk) =>
		buildTransferCall(tk.tokenAddress, recipient, toBaseUnits(tk.amount, tk.decimals))
	);
}
 ...
 */
export function maxNativeSendable(
	balanceWei: bigint,
	reserveWei: bigint,
	decimals: number
): string {
