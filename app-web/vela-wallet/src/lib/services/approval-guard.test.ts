/** Ported from src/__tests__/services/approval-guard.test.ts @ f9bcb278 — vitest; the core is initialised by the server-side wasm init. */
/* eslint-disable @typescript-eslint/no-explicit-any -- the request fixtures are dynamic JSON, as on Expo */
...
describe('approval-guard', () => {
	...
	describe('detectApproval — calldata', () => {
		...
		test('plain ETH send → null', () => {
			expect(
				detectApproval('eth_sendTransaction', [{ to: SPENDER, data: '0x', value: '0x1' }])
			).toBeNull();
		});
	});
	...
	describe('rewriteApprovalParams — calldata', () => {
		test('caps an unlimited approve to a finite amount; spender preserved; no max word remains', () => {
			...
			const out = rewriteApprovalParams(req.method, req.params, d, {
				type: 'amount',
				amountRaw: 500_000000n
			});
			const newData = out[0].data;
			// re-detect the rewritten calldata
			const d2 = detectApproval('eth_sendTransaction', [
				{ to: USDC, data: newData, value: '0x0' }
			])!;
		...
		test('revoke sets amount to 0', () => {
			...
			const d2 = detectApproval('eth_sendTransaction', [
				{ to: USDC, data: out[0].data, value: '0x0' }
			])!;
		...
		test('rewriting to an unbounded amount THROWS', () => {
			...
			const d = detectApproval(...args(req))!;
			expect(() =>
				rewriteApprovalParams(req.method, req.params, d, { type: 'amount', amountRaw: MAX_U256 })
			).toThrow(/Unlimited/i);
			expect(() =>
				rewriteApprovalParams(req.method, req.params, d, {
					type: 'amount',
					amountRaw: UNLIMITED_CAP_256
				})
			).toThrow(/Unlimited/i);
		});
