/** Ported from src/__tests__/services/eip681.test.ts @ f9bcb278 — vitest; the core is initialised by the server-side wasm init. */
...
describe('buildEIP681', () => {
	...
	it('native + amount', () => {
		expect(buildEIP681({ recipient: ME, chainId: 1, decimals: 18, amount: '0.1' })).toBe(
			`ethereum:${ME}@1?value=100000000000000000`
		);
	});

	it('token, no amount', () => {
		expect(buildEIP681({ recipient: ME, chainId: 137, tokenAddress: USDC })).toBe(
			`ethereum:${USDC}@137/transfer?address=${ME}`
		);
	});

	it('token + amount', () => {
		expect(
			buildEIP681({ recipient: ME, chainId: 137, tokenAddress: USDC, decimals: 6, amount: '1.5' })
		).toBe(`ethereum:${USDC}@137/transfer?address=${ME}&uint256=1500000`);
	});

	it('omits amount when zero/empty', () => {
		expect(buildEIP681({ recipient: ME, chainId: 1, decimals: 18, amount: '0' })).toBe(
			`ethereum:${ME}@1`
		);
	});
});
...
describe('parseEIP681', () => {
	...
	it('parses native + amount', () => {
		...
		expect(r).toMatchObject({
			...
			recipient: ME,
			isNative: true,
			amountBaseUnits: 100000000000000000n
		});
	});
	...
	it('accepts the legacy pay- prefix', () => {
		expect(parseEIP681(`ethereum:pay-${ME}@1`)).toMatchObject({
			chainId: 1,
			recipient: ME,
			isNative: true
		});
	...
	it('parses a chainless request', () => {
		expect(parseEIP681(`ethereum:${ME}`)).toMatchObject({
			chainId: undefined,
			recipient: ME,
			isNative: true
		});
...
describe('pay-link origin', () => {
	...
	it('web keeps the current origin (Expo web / self-hosted link back to their own host)', () => {
		...
		(globalThis as { window?: unknown }).window = {
			location: { origin: 'https://wallet.getvela.app' }
		};
	...
	it('native (no window.location) uses the default public host', () => {
		...
		(globalThis as { window?: unknown }).window = {}; // RN defines window but not window.location
		expect(payLinkBase()).toBe(PAY_LINK_FALLBACK);
	});
