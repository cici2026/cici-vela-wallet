/** Ported from src/__tests__/services/batch-send.test.ts @ f9bcb278 — vitest; the core is initialised by the server-side wasm init. */
...
import { toBaseUnits, fromBaseUnits } from './eip681';
import type { APIToken } from './tokens-model';
...
describe('buildSplitCalls (① one token → many recipients)', () => {
	...
	it('supports a native token split', () => {
		const calls = buildSplitCalls({ tokenAddress: null, decimals: 18 }, [
			{ address: A, amount: '1' }
		]);
...
describe('toMultiTokenSpecs', () => {
	...
	it('feeds straight into buildMultiTokenCalls', () => {
		const calls = buildMultiTokenCalls(
			A,
			toMultiTokenSpecs([token({ tokenAddress: USDC, decimals: 6, balance: '1' })])
		);
